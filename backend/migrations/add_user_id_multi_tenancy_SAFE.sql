-- SAFE Migration: Add user_id to tables with existing data
-- This version handles existing data correctly
...
-- 4. Add user_id to settings table (nullable first)
ALTER TABLE settings
ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES users(id) ON DELETE CASCADE;

-- 5. Add user_id to notifications table (nullable first)
...
-- Update existing settings with the first user
UPDATE settings
SET user_id = (SELECT id FROM users ORDER BY created_at ASC LIMIT 1)
WHERE user_id IS NULL;
