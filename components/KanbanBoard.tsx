      ...
      <div className="flex space-x-4 min-w-[1200px] h-[calc(100vh-14rem)]">
        {COLUMNS.map((col) => {
          ...
          return (
            <div 
              key={col.id} 
              className={`flex-1 flex flex-col rounded-xl border ${col.borderColor} bg-gray-50/50 min-w-[280px]`}
              ...
              <div className="flex-1 p-2 overflow-y-auto space-y-3 scrollbar-thin scrollbar-thumb-gray-300">
                {items.map((opp) => (
                  <div
                    ...
                    className={`
                      bg-white p-3 rounded-lg shadow-sm border border-gray-200 cursor-pointer 
                ...
                {items.length === 0 && (
                  <div className="h-24 border-2 border-dashed border-gray-200 rounded-lg flex items-center justify-center">
                    <p className="text-xs text-gray-400 font-medium">Arraste aqui</p>
                  </div>
                )}
              </div>
            </div>
